...
public class CampaignRedeemAccepted {
	...
	//public void campaignaccepted() 
		...
		WebElement selectelement = driver
				.findElement(By
						.xpath("html/body/div[2]/div[5]/div/div/div/div[1]/form/div[3]/select"));
		...
		WebElement selectelement1 = driver
				.findElement(By
						.xpath("html/body/div[2]/div[5]/div/div/div/div[1]/form/div[4]/select"));
		Select dropdown1 = new Select(selectelement1);
		dropdown1.selectByValue("2018");
		// driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
		driver.findElement(
				By.xpath("html/body/div[2]/div[5]/div/div/div/div[1]/form/div[5]/input"))
				.sendKeys("737");
		// driver.manage().timeouts().implicitlyWait(2, TimeUnit.SECONDS);
		driver.findElement(
		...
		try {
			...
			log.info(datemask);
			FileUtils.copyFile(scrFile, new File("//10.91.0.73/Shared/Screenshots/"+datemask+".png"));
...
//change the below query when deployed
	...
	public void browserselect() {
		...
		try {
			...
			rs2 = stmt2.executeQuery("select b.voucher_code as voucher,a.frequency,a.displayprice from nextory_new.campaign_voucher_master a left outer join nextory_new.camp_voucher_map b on a.camp_id=b.camp_id where multiple_reduction_allow=1  and status='ACTIVE' and active=1 and expiry_date>=curdate() and displayprice>0 limit 1;");
			// and displayprice>0 and camp_name='Buzzador_friend'
			while (rs2.next()) {
				voucher = rs2.getString("voucher");
			//	voucher = "0c0bF";
